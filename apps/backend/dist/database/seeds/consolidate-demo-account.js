...
async function consolidateDemoAccount() {
    ...
    try {
        ...
        const accountStats = await Promise.all(demoAccounts.map(async (driver) => {
...
async function ensureCompleteAccount(driverRepo, driver) {
...
async function createSingleDemoAccount(driverRepo) {
    const password = 'Pri@0110';
    const hashedPassword = await bcrypt.hash(password, 10);
    const mockProfileData = {
        password: hashedPassword,

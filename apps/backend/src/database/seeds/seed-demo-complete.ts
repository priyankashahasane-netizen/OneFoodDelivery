...
const AppDataSource = new DataSource({
  ...
  synchronize: false, // We'll use migrations or manual table creation
  logging: true,
});

/**
 * Main seeding function - Creates complete demo data for demo driver
 */
async function seedDemoComplete() {
  ...
  try {
    ...
    // Verify tables exist by checking repositories
    ...
    for (const { name, repo } of tables) {
      try {
        await repo.count(); // Try to count - if table doesn't exist, this will fail
    ...
    // Complete profile data matching OpenAPI DriverProfile schema
    const completeProfileData = {
      password: hashedPassword,
      email: 'demo.driver@example.com',
      // Financial fields (as per OpenAPI DriverProfile)
      balance: 1250.75,
      cashInHands: 125.50,
      ...
      earning: 1, // Enable earnings feature
      // Order statistics
      orderCount: 342,
      todaysOrderCount: 8,
    ...
    if (demoDriver) {
      ...
      demoDriver.online = true; // Set online for demo
      ...
      // Update phone to preferred format if needed
    ...
    // Clear existing orders for demo driver (optional - comment out to keep)
    ...
    const orders = await orderRepo.save([
      ...
      // Picked up orders
      {
        ...
        driverId: demoDriver.id,
        assignedAt: new Date(Date.now() - 600000), // 10 mins ago
        zoneId: null,
      },
      // In transit orders
      {
        ...
        driverId: demoDriver.id,
        assignedAt: new Date(Date.now() - 1200000), // 20 mins ago
        zoneId: null,
      },
      // Delivered orders
      {
    ...
    // ==========================================
    // 4. CREATE ROUTE PLANS
    // ==========================================
    console.log('\n🗺️  Step 4: Creating route plans...');

    // Clear existing route plans for demo driver
    ...
    const routePlans = await routePlanRepo.save(
      assignedOrders.map((order, index) => ({
        ...
        totalDistanceKm: 5.5 + Math.random() * 3, // Random distance between 5.5-8.5 km
        etaPerStop: ['600', '1800'],
    ...
    // ==========================================
    // 5. CREATE TRACKING POINTS
    // ==========================================
    console.log('\n📍 Step 5: Creating tracking points...');

    // Clear existing tracking points for demo driver
    await trackingRepo.delete({ driverId: demoDriver.id });

    const trackingPoints: Partial<TrackingPointEntity>[] = [];
    const now = Date.now();

    // Create tracking points for delivered orders
    ...
    for (const order of deliveredOrders) {
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        trackingPoints.push({
          ...
          recordedAt: new Date(startTime + i * 240000), // Every 4 minutes
        });
      }
    }

    // Create tracking points for in-transit order
    const inTransitOrders = orders.filter(o => o.status === 'in_transit' && o.driverId === demoDriver.id);
    for (const order of inTransitOrders) {
      const startTime = now - 1200000; // 20 mins ago
      const numPoints = 8;
      for (let i = 0; i < numPoints; i++) {
        const progress = i / numPoints;
        const lat = order.pickup.lat + (order.dropoff.lat - order.pickup.lat) * progress + (Math.random() - 0.5) * 0.001;
        ...
        trackingPoints.push({
          ...
          recordedAt: new Date(startTime + i * 150000), // Every 2.5 minutes
        });
      }
    }

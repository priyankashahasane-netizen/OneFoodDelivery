...
/**
 * Demo Data Seeder
 * Creates drivers, orders, route plans, and tracking points for testing
 */
export async function seedDemoData(dataSource: DataSource): Promise<void> {
  ...
  // Note: We don't delete shifts here - they're shared across drivers

  // ==========================================
  // 1. CREATE DEMO DRIVERS
  // ==========================================
  ...
  const drivers = await driverRepo.save([
    ...
    },
    {
      ...
      latitude: 12.9698,
      longitude: 77.6469,
      lastSeenAt: new Date(Date.now() - 1800000), // 30 mins ago
      metadata: {
  ...
  // ==========================================
  // 4. CREATE TRACKING POINTS (GPS TRAILS)
  // ==========================================
  ...
  // Create a GPS trail for delivered order (Order 9)
  const trail1StartTime = now - 3600000; // 1 hour ago
  for (let i = 0; i < 20; i++) {
    trackingPoints.push({
      orderId: orders[8].id,
      driverId: drivers[0].id,
      ...
      heading: 135,
      recordedAt: new Date(trail1StartTime + i * 120000), // Every 2 minutes
    });
  }

  // Create a GPS trail for out_for_delivery order (Order 7)
  const trail2StartTime = now - 1200000; // 20 mins ago
  for (let i = 0; i < 10; i++) {
    trackingPoints.push({
      orderId: orders[6].id,
      driverId: drivers[3].id,
  ...
  // ==========================================
  // 5. CREATE SHIFTS (if they don't exist)
  // ==========================================
    ...
    const shifts = await shiftRepo.save([
      {
        ...
        endTime: '16:00:00',
        status: 1,
        driverId: null, // General shift, available to all drivers
        zoneId: null,
      },
      {

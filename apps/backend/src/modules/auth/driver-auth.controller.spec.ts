...
describe('DriverOtpController', () => {
  ...
  describe('POST /v1/auth/delivery-man/login (Legacy)', () => {
    it('should login with password for existing driver', async () => {
      // Arrange
      const phone = '+919999999999';
      ...
      // Mock bcrypt.compare - this will need to be done with actual bcrypt
      // For testing, we'll assume password matches
      const bcrypt = require('bcrypt');
      jest.spyOn(bcrypt, 'compare').mockResolvedValue(true);

      // Act
      const result = await controller.legacyLogin({ phone, password });

      // Assert
      expect(result.ok).toBe(true);
      expect(result.token).toBe('jwt-token-here');
      expect(result.delivery_man).toBeDefined();
      expect(result.delivery_man.id).toBe('driver-id');
    });

    it('should auto-create driver with default password if not exists', async () => {
      // Arrange
      const phone = '+919876543210';
      ...
      const newDriver = {
        ...
        capacity: 1,
        online: false,
        metadata: { password: '$2b$10$hashed' },
      };
      mockDriversRepository.create.mockReturnValue(newDriver);
      mockDriversRepository.save.mockResolvedValue(newDriver);
      mockJwtService.signAsync.mockResolvedValue('jwt-token-here');
      ...
      const bcrypt = require('bcrypt');
      jest.spyOn(bcrypt, 'hash').mockResolvedValue('$2b$10$hashed');
      ...
      expect(result.ok).toBe(true);
      expect(result.token).toBe('jwt-token-here');
      expect(mockDriversRepository.create).toHaveBeenCalled();
      expect(mockDriversRepository.save).toHaveBeenCalled();
    });
